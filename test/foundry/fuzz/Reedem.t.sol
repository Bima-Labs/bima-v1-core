...
contract Redeem is TestSetUpPSM {
    ...
    function setUp() public virtual override {
        ...
        psm.mint(USER, USER, initalMintToUser);
    ...
    function testFuzz_Redeem(uint256 _underlyingAmount) public {
       _underlyingAmount = bound(_underlyingAmount,1,initalMintToUser);
        ...
        psm.redeem(USER, USER, _underlyingAmount);
        vm.stopPrank();
    ...
    function testFuzz_Redeem_Revert_NotEnoughLiquidty(uint256 amount ) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(BimaPSM.NotEnoughLiquidty.selector, address(mockUnderlyingToken),underlyingTokenBalanceOfPSM,amount)
        );
        psm.redeem(USER, USER, amount);
        vm.stopPrank();
     }
}
